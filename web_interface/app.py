...
_local_cfg = os.path.join(_BASE_DIR, "local_config.py")
_cfg = os.path.join(_BASE_DIR, "config.py")

if os.path.isfile(_local_cfg):
    app.config.from_pyfile(_local_cfg)
else:
    app.config.from_pyfile(_cfg)

# Optional: allow this Flask app to update the quadruped web command
try:
    # Ensure project root is importable when running as `python web_interface/app.py`
    _ROOT_DIR = os.path.abspath(os.path.join(_BASE_DIR, ".."))
    if _ROOT_DIR not in sys.path:
        sys.path.insert(0, _ROOT_DIR)
    from controllers.web_shared_command import set_command as _set_quadruped_cmd
except Exception:
    _set_quadruped_cmd = None
...
def motor():
    ...
    if stickX is not None and stickY is not None:
        ...
        # Update quadruped command (normalized floats) if integration is enabled.
        if _set_quadruped_cmd is not None:
            try:
                _set_quadruped_cmd(float(stickX), float(stickY))
            except Exception:
                pass
